...
export default class CaseList extends React.Component {
  ...
  filterClients(e) {
    ...
    let filteredList = fullList.filter(item => {
      ...
      if (
        item.cli.lastname.toLowerCase().match(text) ||
        item.cli.firstName.toLowerCase().match(text)
      )
        return item;
    });
    if (!text || text === "") {
  ...
  filterCases(e) {
    ...
    let filteredList = fullList.filter(item => {
      // search from a full list, and not from a previous search results list
      if (
        item.cas.casestatus.toLowerCase().match(text) ||
        item.cas.casetype.toLowerCase().match(text)
      )
        return item;
    });
    if (!text || text === "") {
        ...
        {this.state.noData ? (
          ...
          <View style={styles.container}>
            <View style={{ flexDirection: "row", paddingTop: 20 }}>
              <Text
                style={{ paddingLeft: 23, fontSize: 16, fontWeight: "bold" }}
              >
                {" "}
                Case Type:{" "}
              </Text>
              <Text
                style={{ paddingLeft: 93, fontSize: 16, fontWeight: "bold" }}
              >
                {" "}
                Case Status:{" "}
              </Text>
            ...
            <View style={{ flexDirection: "row" }}>
              <Picker
                ...
                style={{ width: 156, height: 56, marginLeft: 20 }}
                itemStyle={{ height: 56, fontSize: 13 }}
              >
              ...
              <Picker
                onValueChange={this.filterCases.bind(this)}
                style={{ width: 156, height: 56, marginLeft: 25 }}
                itemStyle={{ height: 56, fontSize: 13 }}
              >
                ...
                renderItem={({ item }) => (
                  <ListItem
                    title={
                      <View>
                        <Text style={styles.title}>
                          {item.cli.firstName + " " + item.cli.lastname}
                        </Text>
                      </View>
                    }
                    subtitle={
                    ...
                    rightTitle={
                      <Text
                        style={
                          item.cas.casestatus === "Open"
                            ? styles.subtitleBlue
                            : item.cas.casestatus === "Pending"
                              ? styles.subtitleRed
                              : item.cas.casestatus === "Scheduled"
                                ? styles.subtitleGreen
                                : item.cas.casestatus === "Closed"
                                  ? styles.subtitleYellow
